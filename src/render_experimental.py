...
# ==========================================
def generar_animacion_god_mode(ruta_audio, nombre_salida_temp, fps=30, duracion=None, seed=None, allowed_engines=None, use_flash=True):
    ...
    W, H = WIDTH, HEIGHT # Usar configuración global
    aspect_ratio = W / H
    ...
    use_gs = ('GS' in allowed_engines) if allowed_engines else True
    use_lorenz = ('LORENZ' in allowed_engines) if allowed_engines else True
    
    # Fallback: Si todo está desactivado, activar GS por defecto para evitar pantalla negra
    if not use_gs and not use_lorenz:
        use_gs = True
    ...
    try:
        for i in tqdm(range(total_frames), desc="Renderizando Física + Geometría"):
            ...
            if use_gs:
                ...
                # HOMEOSTASIS GS
                if np.mean(V) < 0.001:
                    V[gs_h//2-10:gs_h//2+10, gs_w//2-10:gs_w//2+10] = 0.5
                
                # --- HOMEOSTASIS GLOBAL (GS) ---
                if np.any(np.isnan(V)) or np.max(V) > 10.0:
                    V[:] = 0; U[:] = 1
                ...
                gs_img_color = np.zeros((gs_h, gs_w, 3), dtype=np.uint8)
            ...
            if use_flash:
                frame_final = fx.aberracion_cromatica(frame_final, strength=kick * 10)
...
def unir_video_con_musica(video_path, audio_path, output_path, duracion=None):
    ...
    if duracion:
        # FIX: Compatibilidad con MoviePy v2.0 (subclip -> subclipped)
...
if __name__ == "__main__":
    ...
    CANCION = args.audio
    TEMP = "temp_experimental.mp4"
    FINAL = "Rebeldia_Cosmica_Experimental.mp4"
    
    DURACION_TEST = args.duration
    
    if generar_animacion_god_mode(CANCION, TEMP, fps=30, duracion=DURACION_TEST, seed=args.seed, allowed_engines=args.engines, use_flash=not args.no_flash):
