...
def unir_video_con_musica(video_path, audio_path, output_path, duracion=None):
    ...
    if not os.path.exists(video_path):
        print(f"[X] Error: El archivo de video '{video_path}' no existe.")
        return
    if os.path.getsize(video_path) == 0:
        print(f"[X] Error: El archivo de video '{video_path}' está vacío (0 bytes).")
        return
    ...
    try:
        ...
        if video.duration < audio.duration:
            print(f"[CORTANDO] Ajustando audio ({audio.duration:.2f}s) al video ({video.duration:.2f}s)...")
