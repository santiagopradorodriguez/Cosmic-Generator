...
\title{\Huge \textbf{Cosmic-Generator} \\ \vspace{0.5cm} \Huge \textbf{Manual de Usuario - Volumen 2} \\ \vspace{1cm} \Large \textit{La Enciclopedia Cósmica: Edición Auténtica}}
...
Alan Turing postuló en 1952 que un sistema de sustancias químicas reaccionando y difundiendo en un espacio podía generar patrones espacialmente periódicos a partir de un estado homogéneo debido a la inestabilidad. En Cosmic-Generator usamos el modelo Gray-Scott para materializar esta visión, inyectando la energía del espectro de audio (RMS) en la tasa de alimentación del sustrato, para desestabilizar continuamente el medio al ritmo de la música.

\chapter{Redes Neuronales Artificiales y CPPNs}
Las Redes Neuronales Artificiales (ANNs) nacen de la inspiración biológica del cerebro humano. En \textbf{Cosmic-Generator}, empleamos una variante generativa altamente especializada: las \textbf{Compositional Pattern Producing Networks (CPPNs)}.

\section{Arquitectura del Modo Neural (CPPN)}
Una CPPN es una red neuronal que mapea coordenadas espaciales $(x, y)$ a un valor de intensidad. A diferencia de un perceptrón multicapa clásico que usa la misma función de activación (ej. ReLU) en todas sus capas, una CPPN utiliza un cóctel de funciones matemáticas heterogéneas. Por ejemplo, incluye funciones sinusoidales para generar repeticiones periódicas (texturas), hiperbólicas para transiciones suaves, y funciones gaussianas para estructuras simétricas centradas.

\section{El Modo Neural en Acción}
Nuestro motor inicializa una CPPN de 4 capas ocultas (con 32 neuronas cada una) utilizando pesos completamente aleatorios. No existe una fase de entrenamiento (Backpropagation). En lugar de ello, la red recibe un tensor de entrada de 6 dimensiones: las coordenadas normalizadas del lienzo, la distancia euclidiana al centro (radio), y las coordenadas tridimensionales $(lx, ly, lz)$ del atractor de Lorenz actual.
\part{Ingeniería de Código: Discretizaciones y Fronteras}


\chapter{Discretizaciones Espaciales}
Para simular el continuo universo de las ecuaciones diferenciales en una cuadrícula discreta de píxeles en la pantalla, debemos discretizar los operadores continuos como el Laplaciano ($\nabla^2$).

\section{El Laplaciano Isotrópico}
En el archivo \texttt{nucleo\_visual.py}, empleamos un stencil de convolución de 9 puntos espaciales para aproximar el operador Laplaciano con simetría isotrópica perfecta. Esto evita los artefactos diagonales típicos de los motores de física de 8-bits.
...
lap_u = (0.8 * (U[r+1, c] + U[r-1, c] + U[r, c+1] + U[r, c-1]) + 
         ...
         4.0 * U[r, c]) / (dx_g**2)
\end{lstlisting}
El parámetro crítico aquí es \texttt{dx\_g = 1.2}. Al escalar el denominador artificialmente, "engañamos" a las matemáticas haciéndoles creer que los píxeles están más separados físicamente, lo que engrosa las estructuras de Turing haciéndolas mucho más atractivas visualmente en monitores HD.

\section{Condiciones de Frontera}
El universo simulado es finito. Para gestionar qué ocurre cuando las perturbaciones acústicas llegan a los bordes de la matriz, utilizamos dos estrategias distintas:
\begin{itemize}
    \item \textbf{Fronteras de Dirichlet Cero (Motor de Fuego):} 
    \begin{lstlisting}[language=Python]
out_u[0:2, :] = 0; out_u[-2:, :] = 0
out_u[:, 0:2] = 0; out_u[:, -2:] = 0
    \end{lstlisting}
    En la ecuación Kuramoto-Sivashinsky, matamos la turbulencia asignando el valor 0 en los márgenes. Esto contiene el fuego dentro de la pantalla, evitando oscilaciones en los bordes.
    
    \item \textbf{Fronteras Periódicas (Motores Cuánticos):} En el simulador espectral (\texttt{nucleo\_espectral.py}), la naturaleza de la Transformada Rápida de Fourier (FFT) impone implícitamente condiciones periódicas en los bordes (lo que sale por la derecha, entra por la izquierda), transformando la matriz en un toroide espacial bidimensional.
\end{itemize}

\chapter{Seguridad Numérica: Parches Anti-Clipping}
La inyección dinámica de los graves musicales (RMS y Transitorios) introduce perturbaciones masivas de energía externa dentro de sistemas dinámicos cerrados, lo que matemáticamente garantiza el riesgo de Overflows (valores tendiendo a Infinito o NaN).

\section{Estabilización Asintótica en la Ecuación KS}
La ecuación de Fuego turbulento (Kuramoto-Sivashinsky) contiene un término no-lineal peligroso: $-0.5 |\nabla u|^2$. Un golpe de bombo de alta amplitud puede disparar el gradiente, causando que el paso de integración explícita de Euler explote. Para solucionar esto, aplicamos un "Parche Anti-Clipping" basado en la tangente hiperbólica:
\begin{lstlisting}[language=Python]
# Estabilidad KS: Clamping suave con tanh (evita bloqueos numericos)
val = np.tanh(val / 20.0) * 20.0
\end{lstlisting}
Al escalar y aplicar \texttt{np.tanh()}, aseguramos que las oscilaciones sigan comportándose linealmente cerca del origen (el comportamiento fluido normal), pero se vean frenadas asintóticamente si intentan sobrepasar la barrera crítica de $\pm 20.0$, logrando una simulación indomable pero inmune a crashes.

\section{Saturación Dura en Reacción-Difusión}
Para los modelos químicos (Gray-Scott), la concentración del reactivo $U$ o del producto $V$ es estrictamente física: no pueden existir valores negativos, ni concentraciones superiores al 100\% (1.0).
\begin{lstlisting}[language=Python]
# Clamping seguro estricto
if val_u > 1.0: val_u = 1.0
elif val_u < 0.0: val_u = 0.0
\end{lstlisting}
Cortar la señal abruptamente en $0.0$ y $1.0$ actúa como un limitador analógico implacable contra los impulsos externos de la música.

\chapter{Análisis del Código: Integración del Modo Neural}
Para finalizar, documentamos el corazón del motor \texttt{CPPNEngine}. Este código no usa librerías pesadas como TensorFlow o PyTorch, sino pura multiplicación de matrices vectorizadas (Numpy) para lograr cientos de Fotogramas Por Segundo (FPS) en la CPU.
...
Notamos dos innovaciones profundas: 
1. El parámetro de tiempo \texttt{dt} es dictado por \texttt{audio\_z} (una combinación de la amplitud y frecuencia de la música actual). Cuando el bombo ataca, el tiempo fluye más rápido.
2. Para evitar que el integrador de Euler pierda precisión debido a los enormes pasos de tiempo inducidos por los graves, fragmentamos \texttt{dt} en \texttt{sub\_dt}, permitiendo integrar la curva caótica con pasos pequeños sin importar qué tan fuerte sea el impacto musical.
